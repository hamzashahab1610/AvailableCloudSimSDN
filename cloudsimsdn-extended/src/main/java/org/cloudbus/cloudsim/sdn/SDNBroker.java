...
public class SDNBroker extends SimEntity {
  ...
  protected static Map<String, SDNDatacenter> datacenters = new HashMap<String, SDNDatacenter>();
  ...
  public void printResult() {
    ...
    for (int i = 0; i < SDNBroker.lastAppId; i++) {
      if (groupNumWorkloads[i] != 0) {
        ...
        Log.printLine(
            "Group num: " + i + ", group avg Serve time CPU:" + groupTotalServetimeCPU[i] / groupNumWorkloads[i]);
