...
public class VmAllocationPolicyEx extends VmAllocationPolicy implements PowerUtilizationMaxHostInterface {
  ...
  public VmAllocationPolicyEx(List<? extends Host> list,
    ...
    for (Host host : getHostList()) {
      ...
      // getFreeMips().add((long)
      // PeProvisionerOverbooking.getOverbookableMips((host.getTotalMips())));
      // getFreeBw().add((long)
      // BwProvisionerOverbooking.getOverbookableBw(host.getBw()));
    }
   ...
   * (non-Javadoc)
   * 
   * @see org.cloudbus.cloudsim.VmAllocationPolicy#allocateHostForVm(org.cloudbus.
   * cloudsim.Vm,
  ...
  /**
   * Allocates a host for a given VM. It determines the host by
   * hostSelectionPolicy
   * 
  ...
  protected void reserveResourceForMigration(Host host, SDNVm vm) {
    ...
    Log.printLine(CloudSim.clock() + ": reserveResourceForMigration() " + vm + " MIPS:" + adjustedMips + "(OR:"
        + overbookingRatioMips + ")");
    Log.printLine(CloudSim.clock() + ": reserveResourceForMigration() " + vm + " BW:" + adjustedBw + "(OR:"
        + overbookinRatioBw + ")");
  }
  ...
  private void reserveResource(Host host, SDNVm vm) {
    ...
    if (finaliseResourceAfterMigration((SDNVm) vm)) {
      // VM was in migration, the resource is already reserved during migration
      // preparation process.
    ...
    Log.printLine(
        CloudSim.clock() + ": reserveResource() " + vm + " MIPS:" + adjustedMips + "(OR:" + overbookingRatioMips + ")");
    Log.printLine(
        CloudSim.clock() + ": reserveResource() " + vm + " BW:" + adjustedBw + "(OR:" + overbookinRatioBw + ")");
