...
public class LogPrinter {
  ...
  private static double hostCarbonFootprint;
  private static double switchCarbonFootprint;
  ...
  public static void printEnergyConsumption(List<Host> hostList, List<Switch> switchList, double finishTime) {
    hostEnergyConsumption = 0;
    switchEnergyConsumption = 0;
    hostCarbonFootprint = 0;
    switchCarbonFootprint = 0;
    hostTotalTime = 0;
    hostOverTime = 0;
    hostOverScaleTime = 0;
    vmTotalTime = 0;
    ...
    /*
     * Log.
     * printLine("========== HOST POWER CONSUMPTION calculated based MIPS allocation ==========="
     * );
     * for(Host host:hostList) {
     * // Allocated MIPS based power consumption
     * PowerUtilizationInterface scheduler = (PowerUtilizationInterface)
     * host.getVmScheduler();
    ...
    for (Host host : hostList) {
      ...
      double carbonFootprint = consumedEnergy * 0.35;
      Log.printLine("Host #" + host.getId() + " Carbon Footprint: " + carbonFootprint);
      hostCarbonFootprint += carbonFootprint;
    }
    ...
    for (Switch sw : switchList) {
      ...
      double carbonFootprint = energy * 0.35;
      Log.printLine("Switch:" + sw.getName() + " Carbon Footprint: " + carbonFootprint);
      switchCarbonFootprint += carbonFootprint;
    }
    ...
    for (Host host : hostList) {
      ...
      Log.printLine("Overload Host #" + host.getId() + ": " + overTime + "/" + totalTime + "=" + overPercent
          + "... Scaled Overload duration= " + overScaleTime);
    ...
    for (Host host : hostList) {
      for (SDNVm vm : host.<SDNVm>getVmList()) {
        ...
        double overPercent = (totalTime != 0) ? overTime / totalTime : 0;
        Log.printLine("Vm(" + vm + "): " + overTime + "/" + totalTime + "=" + overPercent
            + "... Scaled Overload duration= " + overScaleTime);
  ...
  public static void printTotalEnergy() {
    ...
    // Log.printLine("Total energy consumed: " + (hostEnergyConsumption +
    // switchEnergyConsumption));

    // Log.printLine("Host (MIPS based) energy consumed:
    // "+hostEnergyConsumptionMIPS);

    Log.printLine("========== CARBON FOOTPRINT ===========");
    Log.printLine("Host Carbon Footprint: " + hostCarbonFootprint);
    Log.printLine("Switch Carbon Footprint: " + switchCarbonFootprint);
    Log.printLine("Total Carbon Footprint: " + (hostCarbonFootprint + switchCarbonFootprint));
    ...
    // Log.printLine("Scaled overloaded: " + (1.0 - (hostTotalTime == 0 ? 0 :
    // hostOverScaleTime / hostTotalTime)));
    // Log.printLine("Overloaded Percent: " + (hostTotalTime == 0 ? 0 : hostOverTime
    // / hostTotalTime));
    ...
    // Log.printLine("Scaled overloaded: " + (1.0 - (vmTotalTime == 0 ? 0 :
    // vmOverScaleTime / vmTotalTime)));
    // Log.printLine("Overloaded Percent: " + (vmTotalTime == 0 ? 0 : vmOverTime /
    // vmTotalTime));
   ...
   * appIdStartTime[i]+
   * ", Finish="+appIdFinishTime[i]+", Rate="+(double)appIdNum[i]/(appIdFinishTime
   * [i] - appIdStartTime[i])+
   * " req/sec, Response time=" + appIdTime[i]/appIdNum[i]);
   * }
   * 
  ...
  public static void printConfiguration() {
    ...
    Log.printLine("overbookingTimeWindowInterval:" + Configuration.overbookingTimeWindowInterval); // Time interval
                                                                                                   // between points
    ...
    Log.printLine("monitoringTimeInterval:" + Configuration.monitoringTimeInterval); // every 60 seconds, polling
                                                                                     // utilization.
    Log.printLine("overbookingTimeWindowNumPoints:" + Configuration.overbookingTimeWindowNumPoints); // How many points
                                                                                                     // to track
    Log.printLine("migrationTimeInterval:" + Configuration.migrationTimeInterval); // every 1 seconds, polling
                                                                                   // utilization.
