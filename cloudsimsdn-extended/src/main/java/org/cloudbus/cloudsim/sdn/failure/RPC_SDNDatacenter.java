...
public class RPC_SDNDatacenter extends SDNDatacenter {
  ...
  private static final int CHECKPOINT_EVENT = 9996;
  ...
  private static final double VM_RECOVERY_DELAY = 0.009;

  private boolean allVmsCreated = false;
  ...
  private Map<String, Double> vmLastCheckpointTime = new HashMap<>();
  private Map<String, Map<Integer, Double>> checkpointProgress = new HashMap<>();
  ...
  private Map<Integer, Boolean> hostPredictedToFail = new HashMap<>();

  public RPC_SDNDatacenter(String name, DatacenterCharacteristics characteristics,
    ...
    this.failurePredictor = new FailurePredictor(getId());

    CloudSim.addEntity(failurePredictor);
  ...
  public void startEntity() {
    ...
    // Register all hosts with the failure predictor
    for (Host host : getHostList()) {
      SDNHost sdnHost = (SDNHost) host;
      failurePredictor.registerHost(sdnHost);
    }

    // Schedule failure and recovery events
  ...
  public void processEvent(SimEvent ev) {
    switch (ev.getTag()) {
      ...
      case CHECKPOINT_EVENT:
        processCheckpoint((String) ev.getData());
        break;
      case FAILURE_PREDICTION_NOTIFICATION:
        handleFailurePrediction((Integer) ev.getData());
        break;
      default:
  ...
  private void handleFailurePrediction(int hostId) {
    ...
    double currentTime = CloudSim.clock();
    Log.printLine(currentTime + ": PREDICTION - Host " + hostId + " is predicted to fail soon!");
    hostPredictedToFail.put(hostId, true);

    for (Vm vm : host.getVmList()) {
      SDNVm sdnVm = (SDNVm) vm;
      String vmName = sdnVm.getName();

      sendNow(getId(), CHECKPOINT_EVENT, vmName);

      Log.printLine(currentTime + ": Triggered preventive checkpoint for VM " +
          vmName + " due to predicted host failure");
    }
  }
  ...
  protected void processCloudletSubmit(SimEvent ev, boolean ack) {
    ...
    for (Vm vm : getVmList()) {
      SDNVm sdnVm = (SDNVm) vm;
      vmLastCheckpointTime.put(sdnVm.getName(), CloudSim.clock());
      checkpointProgress.put(sdnVm.getName(), new HashMap<>());
    }
  }
  ...
  protected void processHostFailure(int hostId) {
    ...
    boolean wasPredicted = hostPredictedToFail.getOrDefault(hostId, false);
    Log.printLine(CloudSim.clock() + ": This failure was " + (wasPredicted ? "predicted" : "not predicted"));
  ...
  protected void processVmDestroy(SimEvent ev, boolean ack) {
    ...
    if (activatedBackupName != null) {
      ...
      if (backupVm != null) {
        ...
        Log.printLine(CloudSim.clock() + ": Error - Activated backup VM not found: " + activatedBackupName);
      }
    } else {
  ...
  private void processCheckpoint(String vmName) {
    SDNVm vm = findVmByName(getVmList(), vmName);

    if (vm == null || vmStatus.getOrDefault(vmName, VmStatus.FAILED) == VmStatus.FAILED) {
      return;
    }

    List<? extends ResCloudlet> cloudlets = vm.getCloudletScheduler().getCloudletExecList();
    Map<Integer, Double> vmCheckpoints = checkpointProgress.getOrDefault(vmName, new HashMap<>());

    for (ResCloudlet cl : cloudlets) {
      Cloudlet cloudlet = cl.getCloudlet();

      double completedSoFar = cloudlet.getCloudletFinishedSoFar();
      vmCheckpoints.put(cloudlet.getCloudletId(), completedSoFar);
      Log.printLine(CloudSim.clock() + ": Checkpoint taken for cloudlet " + cloudlet.getCloudletId()
          + " on VM " + vmName + ", progress: " + completedSoFar + " MI");
    }

    checkpointProgress.put(vmName, vmCheckpoints);
    vmLastCheckpointTime.put(vmName, CloudSim.clock());

    if (vm.getHost() != null && !vm.getHost().isFailed()) {
      SDNHost sdnHost = (SDNHost) vm.getHost();
      double checkpointInterval = sdnHost.getCheckpointInterval();

      send(getId(), checkpointInterval, CHECKPOINT_EVENT, vmName);
    }
  }
  ...
  protected void processNextActivityProcessing(Processing proc, Request reqAfterCloudlet) {
    ...
    if (vm == null || vm.getHost() == null ||
      ...
      if (vm != null) {
        Map<Integer, Double> vmCheckpoints = checkpointProgress.getOrDefault(vm.getName(), new HashMap<>());
        double checkpointedProgress = vmCheckpoints.getOrDefault(cl.getCloudletId(), 0.0);

        totalProcessingDelay += CloudSim.clock() - vmLastCheckpointTime.getOrDefault(vm.getName(), 0.0);

        long remainingLength = cl.getCloudletLength();

        if (checkpointedProgress > 0.0) {
          remainingLength = cl.getCloudletLength() - (long) checkpointedProgress;
        }

        Cloudlet clonedCloudlet = new Cloudlet(cl.getCloudletId(), remainingLength,
