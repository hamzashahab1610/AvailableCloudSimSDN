...
public class SDNHost extends Host implements Node {
  ...
  private double checkpointInterval = 0;
  ...
  public Node getVMRoute(int src, int dest, int flowId) {
    ...
    if (route == null) {
      this.printVMRoute();
      System.err
          .println(toString() + " getVMRoute(): ERROR: Cannot find route:" + src + "->" + dest + ", flow =" + flowId);
    }
  ...
  private PowerUtilizationMonitor powerMonitor = new PowerUtilizationMonitor(
      new PowerUtilizationEnergyModelHostLinear());
  ...
  public void updatePowerConsumption(double logTime, double utilization) {
    powerMonitor.addPowerConsumption(logTime, utilization);
  }
  ...
  public void setPowerModel(double pMin, double pMax) {
    powerMonitor = new PowerUtilizationMonitor(
        new PowerUtilizationEnergyModelHostLinear(pMin, pMax));
  }
  ...
  public void setCheckpointInterval(double checkpointInterval) {
