...
public class RRC_SDNDatacenter extends SDNDatacenter {
  ...
  private static final double VM_RECOVERY_DELAY = 0.04;
  ...
  private Map<String, Map<Integer, Long>> checkpointProgress = new HashMap<>();
  ...
  public RRC_SDNDatacenter(String name, DatacenterCharacteristics characteristics,
  ...
  public void startEntity() {
    ...
    for (Vm vm : getVmList()) {
      ...
      vmLastCheckpointTime.put(sdnVm.getName(), currentTime);
      checkpointProgress.put(sdnVm.getName(), new HashMap<>());

      SDNHost sdnHost = (SDNHost) sdnVm.getHost();
      double checkpointInterval = sdnHost.getCheckpointInterval();
      send(getId(), checkpointInterval, CHECKPOINT_EVENT, sdnVm.getName());
    }
  ...
  private double calculateCheckpointOverhead(SDNVm vm) {
    int vmRamMb = vm.getRam();
    long vmBwBps = vm.getBw();
    double vmUtilization = vm.getMonitoredUtilizationCPU(CloudSim.clock() - 1, CloudSim.clock());

    double vmBwMbps = (double) vmBwBps / 1000000.0;

    double checkpointOverhead = vmRamMb * vmUtilization / vmBwMbps;
    ...
    if (activatedBackupName != null) {
      ...
      if (backupVm != null) {
        ...
        if (failureTime > 0) {
          ...
          int affectedCloudlets = sourceVm.getCloudletScheduler().getCloudletExecList().size() +
              sourceVm.getCloudletScheduler().getCloudletWaitingList().size();

          if (affectedCloudlets > 0) {
            totalProcessingDelay += (downtime * affectedCloudlets);
          }
  ...
  protected void processVmCreate(SimEvent ev, boolean ack) {
    SDNVm sdnVm = (SDNVm) ev.getData();

    String vmName = sdnVm.getName();
    String hostName = vmAllocation.getVmToHostMapping().get(vmName);
    Integer hostId = hostNameToId.get(hostName);
    SDNHost sdnHost = (SDNHost) HostList.getById(getHostList(), hostId);

    super.processVmCreate(ev, ack);

    double checkpointInterval = Math.sqrt(2 * sdnHost.getMtbf() * Configuration.CHECKPOINT_OVERHEAD);

    sdnHost.setCheckpointInterval(checkpointInterval);
  ...
  private void processCheckpoint(String vmName) {
    ...
    Map<Integer, Long> vmCheckpoints = checkpointProgress.getOrDefault(vmName, new HashMap<>());

    for (ResCloudlet cl : cloudlets) {
      Cloudlet cloudlet = cl.getCloudlet();

      long completedSoFar = cloudlet.getCloudletFinishedSoFar();
    ...
    if (vm.getHost() != null && !vm.getHost().isFailed()) {
      ...
      double checkpointOverhead = calculateCheckpointOverhead(vm);
      double checkpointInterval = Math.sqrt(2 * sdnHost.getMtbf() * checkpointOverhead);

      sdnHost.setCheckpointInterval(checkpointInterval);

      if (CloudSim.clock() + checkpointInterval < 310 && checkpointInterval > 0) {
  ...
  protected void processNextActivityProcessing(Processing proc, Request reqAfterCloudlet) {
    ...
    if (vm == null || vm.getHost() == null ||
      ...
      if (vm != null) {
        long lastCloudletCheckpointProgress = checkpointProgress.getOrDefault(vm.getName(), new HashMap<>())
            .getOrDefault(cl.getCloudletId(), 0L);
        long remainingLength = cl.getCloudletLength() - lastCloudletCheckpointProgress;
