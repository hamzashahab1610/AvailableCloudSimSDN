 ...
 * Note that all traffic estimation is calculated within NOS class, not in
 * Switch class.
...
public class Switch implements Node {
  ...
  private PowerUtilizationMonitor powerMonitor = new PowerUtilizationMonitor(
      new PowerUtilizationEnergyModelSwitchActivePort());
  ...
  public void setPowerModel(double pStatic, double pPort) {
    powerMonitor = new PowerUtilizationMonitor(
        new PowerUtilizationEnergyModelSwitchActivePort(pStatic, pPort));
  }
}
